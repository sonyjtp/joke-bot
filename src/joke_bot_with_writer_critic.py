...
NODE_SHOW_FINAL_JOKE = "show_joke"
...
def add_nodes(graph: StateGraph):
    writer_llm = get_llm(writer_model)
    critic_llm = get_llm(critic_model)
    graph.add_node(node=NODE_SHOW_MENU, action=show_menu)
    graph.add_node(node=NODE_UPDATE_CATEGORY, action=update_category)
    graph.add_node(node=NODE_WRITER, action=create_writer(writer_llm))
    graph.add_node(node=NODE_CRITIC, action=create_critic(critic_llm))
    graph.add_node(node=NODE_UPDATE_LANGUAGE, action=update_language)
    graph.add_node(node=NODE_RESET_STATE, action=reset_state)
    graph.add_node(node=NODE_SHOW_FINAL_JOKE, action=show_joke)
...
def add_conditional_edges(graph: StateGraph):
    ...
    graph.add_conditional_edges(
        ...
        path_map={
            ...
            DECISION_APPROVED: NODE_SHOW_FINAL_JOKE
        }
    )
...
def add_edges(graph: StateGraph):
    ...
    graph.add_edge(start_key=NODE_WRITER, end_key=NODE_CRITIC)
    graph.add_edge(start_key=NODE_SHOW_FINAL_JOKE, end_key=NODE_SHOW_MENU)
...
def create_writer(writer_llm):
    def writer_node(state: AgenticJokeState) -> dict:
        ...
        return {"latest_joke": response.content.strip()}
    return writer_node
...
def show_joke(state: AgenticJokeState) -> dict:
    ...
    joke = Joke(text=joke_text, category=state.category)
    print_joke(joke)
    return {
        "jokes": [joke],
        "retry_count": 0,
